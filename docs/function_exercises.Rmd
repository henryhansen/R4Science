...
*Use the steps above to `map()` through the `penguins_rs` and get the slopes of each linear model output.*  

Hint: using `$` will help grab the needed coefficients.  

### Exercise #6  

For this final exercise we'll want to see how far off the original dataset is with the bootstrapped data set. 
